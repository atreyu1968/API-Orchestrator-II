...
- **LitAgents 2.9.5 (Zero-Cascade Corrections)**: Corrections that introduce ANY new issues are automatically REJECTED. No CASCADE system - if a fix causes new problems, it's not a valid fix. Ultra-conservative surgical prompts: max 50 words per patch, no new content, no canonical changes. Patch size validation: patches >100 words or >50% size change are rejected. This prevents the infinite loop where corrections kept generating more issues than they resolved.
